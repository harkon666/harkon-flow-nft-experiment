...
// 1. Definisikan skrip Cadence
//    Skrip ini mengambil alamat & ID, lalu mengembalikan Display struct
const GET_NFT_METADATA_SCRIPT = `
...
interface UseGetAccessoryMetadataProps {
  address: string
  id: number; // Kita terima 'number' dari JS
}

// 3. Buat hook kustom Anda
export function useGetAccessoryMetadata({ address, id }: UseGetAccessoryMetadataProps) {
  const { data, isLoading, error } = useFlowQuery({
    cadence: GET_NFT_METADATA_SCRIPT,
    args: (arg, t) => [
      arg(address, t.Address),
      arg(id, t.UInt64) // Konversi 'number' JS ke 'UInt64' string Cadence
    ],
    query: {
      // Hanya jalankan jika kita punya alamat dan ID
      enabled: !!address && id != null,
      // Opsi ini bagus untuk metadata
  ...
  return {
    display: data, // Akan bertipe: DisplayView | undefined
    isLoading,
    error,
  };
}